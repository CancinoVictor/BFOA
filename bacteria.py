...
class bacteria():
    ...
    #     # Inserta un gap en una posición aleatoria de una secuencia aleatoria
    #     for i in range(len(poblacion)):
    #         bacterTmp = poblacion[i]
    #         bacterTmp = list(bacterTmp)
    ...
    # ========== MÉTODO NUEVO: TUMBO INTELIGENTE ==========
    def tumbo(self, numSec, poblacion, numGaps):
        """
        Sustituye el método tumbo original.
        Inserta gaps en la secuencia con menos gaps en cada bacteria.
        Así los alineamientos se balancean mejor.
        """
        for i in range(len(poblacion)):
            bacterTmp = list(poblacion[i])
            for j in range(numGaps):
                # Busca la secuencia con menos gaps
                gap_counts = [seq.count('-') for seq in bacterTmp]
                seqnum = gap_counts.index(min(gap_counts))
                # Inserta el gap aleatoriamente en esa secuencia
                pos = random.randint(0, len(bacterTmp[seqnum]))
